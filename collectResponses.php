...
<section class="bannerHeader">
   <div class="container">
      <h2 class=""><?=$survey->sections[$sectionIndex]['SectionName']?> Section</h3>
   </div>
</section> 

<section class="sectionPadding">
<div class="container-fluid">
   <form action="results" method="POST" id="submitFormBtn">
	<div class="row">
...
<?php	
	...
	<div class="col-lg-12 text-center">
		<div class="btn-group btn-group-justified">
			...
			<div>
				<button type="submit" class="submitBtnStyle" onclick="$('form').attr('action', 'section-<?=$previousSection?>');">Previous</button>
			</div>
			<?php } ?>
			<?php if ($nextSection != '') { ?>
			<div class="btn-group" role="group">
				<button type="submit" class="submitBtnStyle" onclick="$('form').attr('action', 'section-<?=$nextSection?>');">Next</button>
			</div>
			<?php } ?>
		</div>
		<!-- Show results button if we are on the final section -->
		<?php if ($nextSection == '') { ?>
			<button type="submit" class="submitBtnStyle showResultOnclick" name="viewResult" value="submitresult">View Results</button>
		<?php } ?>
	</div>
	</div>
	...
	function renderQuestion($question) { 
		...
		?>
				...
				<div class="card mt-4  ml-sm-2 ml-xs-0 mr-sm-2 mr-xs-0 text-left">
					<?php if ($question['Type']!='Banner') {?>
					<h6 class="card-header bg-dark"><?=$question['QuestionText']?></h6>
					<?php } ?>
					<div class="card-body p-3 bg-questionHolder">
						<?php 
